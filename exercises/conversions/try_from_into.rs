...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let c: Vec<_> = arr.iter().filter(|x| **x >= 0 as i16 && **x <= 255 as i16).collect();
        match c.len() {
            3 => {
                Ok(Color{red: *c[0] as u8, green: *c[1] as u8, blue: *c[2] as u8})
            }
            _ => {
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        match slice.len() {
            3 => {
                let c: Vec<_> = slice.iter().filter(|x| **x >= 0 as i16 && **x <= 255 as i16).collect();
                match c.len() {
                    3 => {
                        Ok(Color{red: *c[0] as u8, green: *c[1] as u8, blue: *c[2] as u8})
                    }
